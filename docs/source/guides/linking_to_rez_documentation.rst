...
``intersphinx_mapping``. The next two components select the domain and role. Rez
currently publishes ``attribute`` and ``function`` objects for both custom
domains, referenced with the ``attr`` and ``func`` roles respectively.
...
When possible, use the ``rez.utils.docs`` extension from the same rez release as
the documentation inventory being referenced.
